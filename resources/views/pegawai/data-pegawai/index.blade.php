@extends('pegawai.data-pegawai.base')
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        <div class="col-sm-8">
          <a class="btn btn-primary" href="{{ route('pegawai.create') }}">Add Pegawai</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('pegawai.search') }}">
         {{ csrf_field() }}
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($pegawai as $p)
                <tr role="row" class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('pegawai.destroy', ['id' => $p->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden"  name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('pegawai.edit', ['id' => $p->id]) }}" class="btn btn-warning col-sm-5 col-xs-5 btn-margin" style="margin-left: 10px;">
                        Update
                        </a>
            ...
            <div class="row">
        <div class="col-sm-5">
          <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Showing 1 to {{count($pegawai)}} of {{count($pegawai)}} entries</div>
        </div>
        <div class="col-sm-7">
