@extends('admin.input.pengikut.base')
...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/input.css') }}">
        ...
        <h2><b><center>EDIT SPD EVENT</center></b></h2>
        <button onclick="myFunction()">Edit</button>
<!--         <form method="post" action="{{ route('eventpengikut.update', ['id' => $event->id]) }}" onsubmit="return confirm('Apakah Anda Yakin?')">
            <input type="hidden" name="_method" value="PATCH">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input hidden name="status" value="{{ $event -> status}}">
            <button id="valid" style="margin-top: 10px;" type="submit">Validasi</button>
      </form> -->
        <hr style="border-width: 2px; border-color: black;"></hr>

<form method="POST" id="myForm" name="vform" action="{{ route('espd.update', ['id' => $event->id]) }}" enctype="multipart/form-data">
{{ csrf_field() }}
  ...
  <script type="text/javascript">
    ...
    if (val != "Angkutan Darat") {
       $("#TPK").empty();
       $("#TPK").append('<label>Pesawat / KA</label><input style="width:130px;" required type="text" name="n_transport" value="{{$event->n_transport}}" class="form-control"></input><input style="width:120px;" required type="text" name="no_berangkat" value="{{$event->no_berangkat}}" class="form-control"></input><input required id="tkt-ht" type="number" min="0" name="h_tiket" value="{{$event->h_tiket}}" class="form-control"></input><input style="width:130px;" required type="text" name="nk_transport" value="{{$event->nk_transport}}" class="form-control"></input><input required type="text" name="no_kembali" value="{{$event->no_kembali}}" style="width:120px;" class="form-control"></input><input required id="tkt-htp" type="number" name="h_tiketp" value="{{$event->h_tiketp}}" min="0" class="form-control"></input><input required id="tkt-tht" name="t_transport" value="{{$event->t_transport}}" class="form-control prc" min="2"></input>');          
